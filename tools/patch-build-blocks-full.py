...
# 找到 getBlocks 函数内部并重构为纯净 6 大工业板块
start_marker = "  return ["
end_marker = "  ];\n}\n\n/**\n * Render the visual gallery landing page"

idx1 = text.find(start_marker)
idx2 = text.find(end_marker)

if idx1 != -1 and idx2 != -1:
    six_blocks_code = """  return [
    {
      code: 'na',
    ...
    text = text[:idx1] + six_blocks_code + text[idx2:]
    with open('build.mjs', 'w', encoding='utf-8') as f:
        f.write(text)
    print("Replaced getBlocks with exactly 6 blocks!")
else:
    print(f"Could not find indices: idx1={idx1}, idx2={idx2}")
