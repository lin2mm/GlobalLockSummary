...
# 1. 彻底解决 8 大板块标题定义：拆分为标准中文主名与纯正英文子名
# 同时保留完整的 shortTitle，保证所有子页导航与面包屑完全正确
old_block_pattern = r"    \{\s*code: 'na',.*?tag: isZh \? '⭐ 极强相关 · 南亚非洲基准' : '⭐ South Asia Africa Baseline'\s*\},?\s*items: items\.filter\(i => i\.block === 'af-sa'\)\s*\}"

new_blocks = """    {
      code: 'na',
      title: isZh ? '🇺🇸 北美标准板块' : '🇺🇸 North America',
      titleEn: 'Americas — ANSI / BHMA',
      ...
      hero: {
        title: isZh ? '核心改装基准：美标单缸插销死锁 (Schlage B60)' : 'Core Retrofit Baseline: ANSI Deadbolt (Schlage B60)',
    ...
    {
      code: 'europe5',
      title: isZh ? '🇪🇺 欧陆五国板块' : '🇪🇺 Continental Europe',
      titleEn: 'Continental Europe — DIN / EN',
      ...
      hero: {
        title: isZh ? '核心改装基准：欧标槽型双锁芯 (DIN 18252)' : 'Core Retrofit Baseline: Euro Profile Double-Cylinder (DIN 18252)',
    ...
    {
      code: 'uk-anz',
      title: isZh ? '🇦🇺🇬🇧 澳洲与英国板块' : '🇦🇺🇬🇧 Australia & UK',
      titleEn: 'Pacific & UK — AS / BS',
      ...
      hero: {
        title: isZh ? '核心改装基准：澳式表面安装双扣死锁 (Lockwood 001)' : 'Core Retrofit Baseline: Lockwood 001 Rim Deadlatch',
    ...
    {
      code: 'jp-kr',
      title: isZh ? '🇯🇵🇰🇷 日韩精工板块' : '🇯🇵🇰🇷 Japan & Korea',
      titleEn: 'Japan & Korea — JIS / KS',
      ...
      hero: {
        title: isZh ? '核心改装基准：日本 MIWA 13LA / B5 防犯旋钮锁' : 'Core Retrofit Baseline: MIWA 13LA / B5 Thumbturn',
    ...
    {
      code: 'sea',
      title: isZh ? '🇸🇬🇲🇾 东南亚板块' : '🇸🇬🇲🇾 South East Asia',
      titleEn: 'ASEAN / SEA — SS / MS',
      ...
      hero: {
        title: isZh ? '核心改装基准：新加坡 HDB 铁闸双门联动锁' : 'Core Retrofit Baseline: Singapore HDB Gate Mortise',
    ...
    {
      code: 'gcc',
      title: isZh ? '🇦🇪🇸🇦 中东海湾板块' : '🇦🇪🇸🇦 Middle East GCC',
      titleEn: 'Middle East / GCC — SASO / BS / EN',
    ...
    {
      code: 'latam',
      title: isZh ? '🌎 拉美新兴板块' : '🌎 Latin America',
      titleEn: 'Latin America — ABNT / IRAM',
      ...
      hero: {
        title: isZh ? '核心改装基准：巴西 ABNT 极窄背距插芯锁 (PADO)' : 'Core Retrofit Baseline: Brazil ABNT Narrow Mortise (PADO)',
    ...
    {
      code: 'af-sa',
      title: isZh ? '🇮🇳🇿🇦 非洲与南亚板块' : '🇮🇳🇿🇦 South Asia & Africa',
      titleEn: 'South Asia & Africa — BIS / SABS',
...
match = re.search(old_block_pattern, code, re.DOTALL)
if match:
    code = code[:match.start()] + new_blocks + code[match.end():]
    print('Blocks successfully redefined with distinct title and titleEn.')
else:
    print('Block pattern not matched, attempting fallback replace.')

# 2. 首页卡片模板：中文一行（大字号醒目），英文规范换行单独一行（副标字号），彻底解决中英文单行挤压混乱！
old_portal_card = r"""      <div class="gallery-portal-card__body">
...
new_portal_card = """      <div class="gallery-portal-card__body">
        <div style="display: flex; justify-content: space-between; align-items: baseline; gap: 6px; margin-bottom: 3px;">
          <h2 class="gallery-portal-card__name" style="margin: 0; font-size: 1.05rem; font-weight: 800; color: #0f172a; line-height: 1.3;">${escapeHtml(b.title)}</h2>
          <span style="font-size: 0.70rem; color: #64748b; font-weight: 700; white-space: nowrap; background: #f1f5f9; padding: 2px 6px; border-radius: 4px; border: 1px solid #e2e8f0;">${b.items.length} ${isZh ? '款' : 'models'}</span>
        </div>
        ${b.titleEn ? `<div class="gallery-portal-card__suben" style="font-size: 0.74rem; font-weight: 600; color: #64748b; letter-spacing: 0.02em; margin-bottom: 6px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" title="${escapeHtml(b.titleEn)}">${escapeHtml(b.titleEn)}</div>` : ''}
        <div class="gallery-portal-card__baseline" style="color: #0B1D47; font-weight: 600; font-size: 0.82rem; line-height: 1.35; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; padding-top: 5px; border-top: 1px dashed #e2e8f0;" title="${escapeHtml(b.hero.title.replace(/^[^：:]*[：:]/, '').trim())}">
          <span style="color: #64748b; font-weight: normal; font-size: 0.72rem;">基准: </span>${escapeHtml(b.hero.title.replace(/^[^：:]*[：:]/, '').trim())}
        </div>
      </div>"""

code_updated = re.sub(old_portal_card, new_portal_card, code, flags=re.DOTALL)
if code_updated != code:
    code = code_updated
    print('Card body updated to Chinese top line + English subtitle line.')
else:
    print('Card body regex NOT matched directly.')

with open('build.mjs', 'w', encoding='utf-8') as f:
    f.write(code)

print('build.mjs successfully updated.')
