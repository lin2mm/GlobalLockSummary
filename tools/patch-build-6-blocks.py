...
# 1. 替换 getBlocks 定义，构建完美的 6 大工业板块体系
old_blocks_pattern = """    {
      code: 'sea',
      title: isZh ? '🌏 东南亚与日韩板块 (Asia-Pacific — JIS / SS / KS)' : '🌏 Asia-Pacific (JIS / SS / KS)',
      shortTitle: isZh ? '亚太日韩板块' : 'Asia-Pacific',
      subtitle: isZh ? '亚洲高密度高增长五金体系 · 日本 MIWA / GOAL 超薄锁体、新加坡组屋 HDB 外铁闸双门重合与韩国无锁孔防盗系统' : 'High-density Asian systems · Japan MIWA/GOAL, Singapore HDB double-gates & Korea systems',
      hero: {
        ...
        tag: isZh ? '⭐ 极强相关 · 亚太改装基准' : '⭐ Asia-Pacific Baseline'
      },
    ...
    },
    {
      ...
      hero: {
        title: isZh ? '核心改装基准：巴西 ABNT NBR 14913 极窄背距插芯锁' : 'Core Retrofit Baseline: Brazil ABNT Narrow-Backset Mortise (PADO)',
...
if old_blocks_pattern in text:
    text = text.replace(old_blocks_pattern, new_blocks_pattern)
    print("Replaced old blocks with 6-block golden matrix in build.mjs!")
else:
    print("Old blocks pattern not found directly, checking variations...")

# 2. 更新 categories 注册列表
text = text.replace(
...
print("build.mjs successfully updated with 6 industrial divisions!")
